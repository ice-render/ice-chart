...
- **真机验证脚本必须写断言，不许只打印**（2026-09-25 踩过，代价是一次线上手感 bug）：
  viewport 平移的位移被累积成平方增长（一次拖 80px、画面跑 360px），而我的验证脚本只
  `console.log` 了 tx / ty —— **那个明显异常的数就摆在输出里**，没有断言就没人发现。
  两条纪律：① 每条检查都要 `✓/✗` 断言（含「拖回起点必须回到原位」这类能抓住累积的判据）；
  ② **先交叉验证尺子本身** —— 同一组参数在临时脚本与真实页面上必须量出同一个数，
  同一天两把临时尺子（力参数扫描、标签重叠统计）各算错过一次，都是先信了脚本才发现。
- **别拿元素截图当尺子**：示例页的 canvas 可能被 CSS 拉伸（元素盒 960×510、背衬 960×480），
  `locator.screenshot()` 会跟着拉伸，看着「内容出血」而实际几何是好的。要量像素就读
  `canvasEl.toDataURL()`（背衬原始像素）。
...
**改示例页样式 / 加示例页之后，跑一遍排版门禁**（2026-09-25 立）：

```bash
node scripts/audit-pages.mjs          # 默认 1280 视口；WIDTH=1024 可换
```

它量「画布右缘 vs 卡片内容区 vs 视口」，**画布溢出卡片即失败** —— 这类问题的根因在
页面排版（卡片被压得比画布窄），不在图表里，图表自己的门禁（含 `audit-bleed`）一把都照不到。
设计宽大于视口的页（graph / animation / 大屏）只提示「横向滚动」，不算失败。
...
## 示例页的排版（`examples/*.html` + `assets/style.css`，改样式前必读）

- **卡片不许比画布窄**（2026-09-25 修，别改回去）：画布是**固定内建尺寸**（示例页刻意不做 CSS
  缩放 —— 缩放会让命中检测整体偏移，见下一节），所以 `.card` / `.row .card` 的 `min-width`
  必须不小于内容宽度（现在写的是 `fit-content`）。早先 `.row .card` 写的是
  `flex: 1 1 420px; min-width: 380px`：1280 视口下卡片被压到 598px，620px 的画布溢出卡片 47px、
  被窗口右缘切掉（右图切掉一半）—— 表现是「图表突破了右侧边缘」，而图表本身没问题。
  放不下时正确的行为是**换行成一列**（`.row` 已经 `flex-wrap: wrap`），不是裁图。
- **画布宽度 ≥ 设计宽的页面会横向滚动**（graph 1560 / animation 1260 / 大屏 1572）：这是有意的
  —— 设计宽就是设计宽，宁可滚动也不缩图。排查这类问题别只看画布：先看
  「画布右缘 vs 卡片内边 vs 视口」三个数，`node scripts/audit-pages.mjs` 就是干这个的。
- **审计脚本的视口宽度必须 ≥ 内容设计宽**（既有纪律）：1280 视口下两卡并排会换成上下排列，
  探针按画布左上角分数定位，仍能工作，但截图与人工复核的构图会变。
...
## 桑基节点拖拽重排（改 `layout/sankey.ts` / `SankeySeries` 前必读）

1. **只钉被拖过的那一列**：顺序存在 `sankey.nodeOrder`（节点名或下标），列内的钉法在
   `layoutSankey` 里 —— 表里列到的排在前、没列到的排其后并保持原有相对顺序；整张表缺席时
   **逐值等于以前的行为**（这条由 `tests/layout/sankey.test.ts` 盯着）。
2. **顺序看节点中心，不看上沿**：节点被夹在绘图区里，用户眼里「它现在在哪」就是中心；
   按上沿比会让高节点拖到底时仍排在最前 —— 看到的落点与排出来的顺序对不上。
   （几何限制也是真的：只有矮节点的中心能越过邻居，这是布局按高度铺满一列的必然结果，不是 bug。）
3. **拖出去又拖回来 = 没动过**：松手时的列序与按下时一致就什么都不写、什么事件都不发 ——
   否则会留下一次「隐形钉住」，那一列从此不再跟着松弛自动排（与「拖回起点必须回到原位」同一条纪律）。
4. **顺序必须落到 option 上**（就地改，与 `appendData` 同一口径）：`getOption()` / 快照 /
   `restore()` 都靠它；只改组件内部的布局，重开一次就回到老顺序。测试里要给每张图**自己的**
   option 副本，共用模块级常量会互相污染（踩过）。
5. **关掉就不该有新行为**：`sankey.draggable: false` 时按下节点不进入拖拽。
   回归点：`tests/layout/sankey.test.ts`（列内排序规则）、`tests/chart/sankey.test.ts`
   （拖拽 / 拖回 / 夹取 / 关开关 / 快照往返），真机在 `scripts/audit-interactions.mjs` 的
   `12-sankey-drag-reorder`（拖完同时断言列序、option 与像素）。
...
- **浏览器实测门禁**：`npm run audit:hover -- ./.hover-sweep`
  ...
  否则会把「数据更新」误判成「悬停丢失」。
- **墨迹越界门禁**：`node scripts/audit-bleed.mjs ./.audit-bleed`
  （写完 `npm run build && npm run examples:prepare`，示例服务在跑）
  逐页量**整幅墨迹离画布四边的余量**，贴边（默认 ≤2px）即失败 —— 贴边就等于内容被裁。
  这一类**原来两把尺子都看不见**：`audit-space.mjs` 统计墨迹时只扫绘图区 ±8px 以内（它量空间利用率），
  `audit-interactions.mjs` 的越界探针只查左右轴带的**饱和色**墨迹（文字是灰阶）。
  立这道门禁的起因是力导向图的节点标签：四个大屏的力导向面板底部余量只剩 1~2px，
  `dashboard-energy` 那张的节点名被裁成半行。
- **标签必须夹进自己的盒，且撞了要换边**（2026-09-25）：按固定偏移画在数据旁边的文字
  （`graph` 的节点名）必须走 `placeGraphLabels()` 那类落点计算 —— 先下方、贴底翻上方、
  与已放置的标签相撞就挪到左右侧，最后才选撞得最少的；宽度用 `measureTextWidth` 量真实文字。
  **不许直接 `node.y + size/2 + 9`** —— 那种写法在节点靠近绘图区边缘时会把文字画到画布外。
  注意这与「饼图引导线标签」不冲突：那些标签**本来就该**在绘图区外，但同样不许被画布裁掉。
- **别把「名字能不能读」押在力参数上**（2026-09-25）：实测同一组力参数（`repulsion` / `edgeLength`）
  在浏览器与 Node 里结果能差一个数量级（浏览器里 189 个节点挤成一团、Node 里铺得很开），
  而且最优解是窄带（`edgeLength` 240 → 标签零重叠、200 → 4 对、`repulsion` 300000 → 退化到 1040 对）。
  兜住可读性的是**标签自己找位置**，力参数只负责大致铺开。
...
桑基图的**节点拖拽重排**（列内上下拖，顺序写回 `sankey.nodeOrder`）。
标注（`option.annotation`：目标线 / 阈值线 / 异常点 / 目标区间 —— 声明式图层，不是新系列类型）。

仍未实现（改动前先确认是否要做）：

- 桑基节点的**折叠**（拖拽重排已实现）—— 桑基是 DAG，多父节点的折叠语义不唯一
  （「折叠中间节点后上游下游怎么接」没有唯一答案）；要「只看一部分」就在数据侧过滤 `nodes` / `links`；
- 隐函数 / 等值线（`f(x,y) = 0`，需要 marching squares）。
- **地图明确不做**（用户已明确否决，体量与定位都不合适）；关系数据一律用力导向关系图表达。

**已结案（别再当待办盘一遍）**：

- `appendData` 的「增量绘制」（现在是全量重建像素缓存）：同步流水线只占一帧的 **2%~5%**，
  瓶颈是浏览器**光栅化**（`(program)` 占 90.4%），归一化 / 布局方向再抠都是 0.1 ms 量级 ——
  属**引擎重绘策略**（跨仓，动 `ice-render`），本仓不再立项。数据见
  `plans/incremental-pipeline.md` 的「第 8 期」。
