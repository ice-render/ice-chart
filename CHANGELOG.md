...
- **dev 依赖对齐 `ice-render ^2.11.3`**（peer 范围不变）：引擎这次修的是**连线的命中语义** ——
  ...
  （不再被 14~24px 的宽容差误判成"在线上"）。本版本不含图表层功能变更，只是把这条修复带上并把
  本地开发固定在 2.11.3。
- 门禁（在 2.11.3 上重跑）：单测 40 套件 / 379 用例、36 个示例页冒烟（无 pageerror + 画布有输出）、
