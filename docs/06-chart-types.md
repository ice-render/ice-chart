...
2. **桑基**（`layout/sankey.ts`）：连线按宽度着色；节点可在**自己那一列**里拖拽重排 ——
   松手后按节点**中心**算出该列的新顺序、写回 `sankey.nodeOrder` 并重跑布局，只钉被拖过的那一列
   （列内排序规则见 `layoutSankey` 的注释）。**折叠不做**：桑基是 DAG，多父节点的折叠语义不唯一，
   要「只看一部分」就在数据侧过滤 `nodes` / `links`。
...
3. **y 方向的滑块**（`dataZoom.sliderY`，2026-09-25）：绘图区**右侧外**一条竖直轨道，
   拖动改 y 的数据窗口（走 `setDomainYFromFractions` / `domainYFractions`，与 x 那条同一套口径）。

   - **默认不显示**：多让出一条竖直轨道会改绘图区宽度，不该让既有图的外观凭空变一次；
     写 `sliderY: {}` 就是打开，`show: false` 显式关掉。轨道宽度 `width` 默认 26。
   - **方向约定：上 = 大值、下 = 小值**（与屏幕同向），所以「从上往下拖」= 窗口朝小值走 ——
     与横向那条的符号正好相反，别照抄。
   - **多 y 轴**：`sliderY.axisIndex` 选驱动哪一根（默认 0）。
   - **类目 y 轴不出竖直滑块**：归一化有一条既有规则 —— 类目 y 轴不吃 y 视窗
     ...
     「窗口一个横向柱排行榜」是另一件事（要动那条归一化规则），没做。
   - 兜底与 y 的既有手势（滚轮 / 平移）**同口径**：数值轴最小跨度 0.1% 且不越出数据域、
     拖动另有 0.02 的最小窗口 —— y 侧**没有** x 那套「至少盖住 2 个数据点」的探针
     （那依赖 `xValueAt` 的单调列，y 侧每拖一次就是一趟全量扫描）。
