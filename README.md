...
| 大数据 | LTTB 降采样 + 二分命中 + 列存（虚拟）系列 | 5 万点 × 3 系列构建 35ms，每条曲线只绘制约 2 点/像素；**100 万点散点**开 `virtual: true` 后堆 2.2MB、命中 0.00ms —— [机制与复用边界](./docs/10-column-store-virtual-series.md) |
...
| `scatter` / `line` / `area`（大数据） | `virtual: true` + `data: { x, y }`（列式，推荐 `Float64Array`） | 列存（虚拟）系列：百万级点不建数据点对象与按点缓存，堆 2.2MB / 命中 0.00ms（折线按像素列保留极值）。代价（不进快照、`params.data` 为空）见 [机制与复用边界](./docs/10-column-store-virtual-series.md) |
...
**配色按规则**（节点 = 与贾家的关系：主家两府 / 姻亲四家同一色相按往来排明度 / 亲眷与外人灰阶；
连线 = 关系类型：血脉 / 姻缘与情感 / 主仆与师承 / 交游，粗细 = 亲疏，页面带图例）；
...
**这一节只回答「还没做什么」。** 「已经有什么」以 [能力清单](#能力清单) 与
[图表类型与写法](#图表类型与写法) 为准；纪律条目与「已实现 / 未实现」的单一来源是
[`AGENTS.md`](./AGENTS.md)，施工图在 [`plans/`](./plans)（性能那条是 `plans/incremental-pipeline.md`）。
同一份清单抄成三份的结果就是各自过期 —— 比如「把列存推广到折线 / 面积与热力图」在这里挂了一阵子，
而它早已落地（列存折线与列存热力矩阵见上面的「图表类型与写法」与
[机制与复用边界](./docs/10-column-store-virtual-series.md)）。
...
- **标注的第二梯队**：非直角坐标系里的标注（树图 / 日历热力 / 雷达 …）——
  ...
  另有趋势线与误差棒（按需）。
- **隐函数 / 等值线**（`f(x, y) = 0`，需要 marching squares）：属 B 档，要做就另开一个包，
  不进核心库。

明确不做：

- **地图 / 地理**：地理数据 + 投影 + 交互是另一个体量。关系数据用力导向关系图表达；
  需要地图的应用走 `registerSeriesType()` 自定义系列口。
- **3D**：引擎能力之外，另立项目才成立。
- **K 线 / 交易语义、甘特 / 排期、建模类图示（ER / 流程 / BPMN / UML / 状态机…）**：
  由家族里专门的包承担（它们挂在本库之上，本库不感知）；冷门形态一律走自定义系列注册口。
