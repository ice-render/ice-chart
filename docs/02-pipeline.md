...
- **`show:false` 的 x 轴也要出抽稀表**（只是不占位、不绘制）：多 pane 里上面几块常藏掉 x 轴，
  网格却要和下面那块对齐成方格。
- **首末标签要占位，不许推动**：刻度标签居中画在刻度上，末刻度落在绘图区右沿时有一半探到绘图区
  外面，只靠固定的 `margin.right` 会让它顶到画布边（小面板实测只剩 7px）。`xAxis.edgeLabelPadding`
  （默认 12px）把「探出去的那半截 + 留白」提前从绘图区里扣掉 —— **只在不够时扣**，
  所以在绝大多数图上布局逐像素不变；**不能改成「把标签往里推」**：推了会压住相邻那颗
  （见 `thinXAxisLabels` 里那条实测）。
