...
export interface InternalAxis {
  ...
  position: 'left' | 'right';
  /**
   * 这根轴的域**是从数据类目推出来的**，不接受 y 视窗。
   *
   * 两个来源：横向图的类目 y 轴（值在 x 轴上）、热力图的行类目。
   * 「带视窗重跑一遍归一化」那条路对它们本来就是跳过 y 视窗的，
   * 事后套视窗（`applyViewToNormalized`）必须照同一条规则走，否则
   * 类目域会被视窗里的数值覆盖（横向柱子的类目带、热力图的行会整块塌掉）。
   */
  categoryDomain?: boolean;
  /**
