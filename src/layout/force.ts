...
/** `graph.label` 的形状（从 option 里取，避免 types 循环依赖）。 */
type GraphLabelOption = NonNullable<GraphOption['label']>;
 ...
 *
 * 节点直径是「权重 → `symbolSizeRange`」映射后的结果，所以 `minSize` 直接写像素即可，
 * 与 option 里的量纲一致（不用再换算成权重）。
 */
export function shouldLabelGraphNode(size: number, label?: GraphLabelOption): boolean {
  if (!label) return true;
  if (label.show === false) return false;
  const minSize = Number(label.minSize);
