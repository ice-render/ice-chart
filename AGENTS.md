...
**技术与架构文档在 `docs/`**（索引 `docs/README.md`：架构与分层 / 更新流水线 / 渲染与组件契约 /
交互与命中 / 数据形态与大数据 / 图表类型与场景坐标 / 主题与无障碍 / 测试与发版）。
本文件是**纪律条目**（踩过的坑与不许改回去的地方），`docs/` 是**机制说明与边界**；
改代码时两边一起更新 —— 新增纪律写这里，新增机制写那边。
...
实测（1M 根窗口、真机 Chromium、宿主是 ice-trading-chart 的流式页）——**函数级**已经拿掉的部分：

| 项目 | 前 | 后 |
|---|---|---|
| 柱子绘制（`fill` + 每根一个渐变 + `barRectAt`） | 1688ms + 180ms + 167ms（每 3 秒） | **归零**（每像素列一个矩形、柱高 <3px 纯色） |
| K 线压缩聚合（`renderCompact`） | 848ms / 3 秒（逐根扫 40 万根） | **64ms** |
| 虚拟折线的逐点像素缓存（`rebuildPixels`） | 958ms / 3 秒（1M 点白建一趟） | **归零**（虚拟系列按需算像素） |

⚠️ **帧时间要按下面这个口径看**（2026-09-24 更正）：1M 规模下**帧**由**流水线**主导 ——
每帧要跑 4 次归一化（2 个系列 × `applyOption` + `rebuild` 各一遍），而每次归一化都要把
**两张不同的类目表**合并一遍（1M × 2 次 `String()` + 哈希 ≈ 38ms/次）。所以：
`#stat-frame` 在「K 线 + 量柱 + 均线」下是 **750ms/帧**（改这一轮之前 ≈2.2s、更早 ≈3.5s），
而不是「60fps」。K 线单系列才是 16.7ms/帧（CPU 1.4s/3 秒，有余量）。
（早先写成「3516ms → 16.7ms」是**探针测错了**：rAF 采样撞上停摆的推流循环。现在探针先确认
「窗口仍在推进」再采帧 —— 这条经验值得所有性能探针抄走。）
...
## 分布组图（`violin` / `beeswarm`，改动前必读）

1. **密度在数据空间算，避让在像素空间算**。核密度只跟观测值有关，放归一化里
   （纯函数 `src/layout/density.ts`，结果挂到数据点的 `violin` 字段，可单测）；
   蜂群避让算的是「两个半径 r 的圆会不会压在一起」，必须知道比例尺，
   所以放在组件的 `rebuildPixels()`。反过来做都得自己造一套坐标，缩放一动手感立刻崩。
2. **轮廓必须整个落在 y 域里**：密度网格向样本极值外各延伸 3 个带宽，进域取的是
   **网格端点**（`p.violin.grid[0]` / 末项），不是观测极值 —— 用观测极值会让两端的尖角
   被绘图区边缘切平（改回去就是回归）。
3. **蜂群宽度必须封顶，退化必须确定性**：单侧最大偏移取
   `min(band × spread / 2, step/2 − r)`；槽位排不下时用确定性哈希散开，
   **不要引随机源** —— 同一份数据每次画出来必须一样，否则序列化往返的「像素一致」契约直接破。
4. **命中与渲染共用同一份几何**：小提琴命中是轮廓多边形内部（偶奇规则），
   蜂群命中是像素最近邻；两者都读 `rebuildPixels()` 填的像素缓存，不许另算一套。
5. **锚点必须落在自己的命中区里**（2026-09-25 修，别改回去）：小提琴的像素缓存的 y
   取的是**峰值密度那一行**（轮廓最宽处），**不是中位数**。双峰分布的中位数落在两个峰之间的
   谷底 —— 那里核密度≈0、轮廓宽度收成 0，锚点就跑到自己的多边形**外面**：图上明明看得见，
   却悬停不上，提示框与高亮也锚在形状外（正是「看得见的点」与「点得到的点」分叉）。
   单峰分布里中位数本来就靠近峰，两种取法看不出差别，所以这个坑只在双峰上暴露 ——
   实测 `distribution` 页的「双峰（带宽 ×0.3）」探针 `hitTestIndex` 返回 -1、同一组最宽处返回 1。
   回归点：`tests/chart/distribution-series.test.ts` 的「双峰分布的锚点仍在自己的轮廓里」。
...
9. `src/a11y.ts`：`SERIES_ROLE_HINT` 是 `Record<SeriesType, string>`，新类型漏了这一条
   **`npm run types:check` 直接编译不过** —— 它不在图表链路上，最容易漏（2026-09-24 实测）。
...
已实现：直角坐标 / 极坐标（饼图、玫瑰图）/ 雷达图 / 桑基图；多 y 轴；
dataZoom 滑块（**x 在底部、y 在绘图区右侧**：`dataZoom.sliderY`，默认不显示）；
LTTB 降采样；无障碍数据表镜像；热力图、桑基图；箱线图、瀑布图、矩形树图、漏斗、仪表盘、水位球；
**六边形分箱（`hexbin`：像素空间分格、O(1) 命中、交互单位是格子）**；
**日历热力（`calendar`：UTC 日期语义、反解命中）、多轴分类流（`alluvial`：表的分面 + 重心排序）**；
**分布组图（小提琴 `violin` / 蜂群 `beeswarm`，与箱线同轴叠成雨云图 —— 密度是纯函数、
避让在像素空间，见 `docs/06-chart-types.md` §2.1）**；
